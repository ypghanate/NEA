...
def audio_to_midi(audio_path, output_midi_path):
    ...
    # Rename the first generated MIDI file to match output_midi_path
...
def convert_audio_sheet(midi_path, user_skill, model_path=None):
    roll = midi_to_pianoroll(midi_path)
    sheet_diff = overallComplexity(midi_path, roll)

    # Map difficulty vs user skill
    ...
    with torch.no_grad():
        output_roll = model(roll_tensor).squeeze(0).numpy()
    output_roll = (output_roll > 0.5).astype(float)

    base = os.path.splitext(os.path.basename(midi_path))[0]

    # Save files in MEDIA_ROOT/uploads/sheet_music
